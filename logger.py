...
class TradeLogger:
    ...
    def calculate_performance_metrics(self, initial_balance: float) -> Dict:
        ...
        realized_trades = trades_df[trades_df['pnl'] != ''].copy()
        
        if len(realized_trades) > 0:
            realized_trades['pnl'] = pd.to_numeric(realized_trades['pnl'])
    ...
    def save_metrics(self, initial_balance: float):
        ...
        if metrics:
            ...
            print(f"Sharpe Ratio: {self._colorize_text(f'{metrics['sharpe_ratio']:.3f}', sharpe_color)}")
