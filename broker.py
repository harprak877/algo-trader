...
class PaperBroker:
    ...
    def get_account_info(self) -> Dict:
        ...
        try:
            ...
            # Get account details using the direct Alpaca format
            info = {
            ...
            self.logger.info(f"Account info: ${info['buying_power']} available as buying power")
...
class AlpacaBroker:
    ...
    def __init__(self, config: Dict):
        ...
        try:
            # Initialize TradingClient - paper=True automatically uses correct endpoint
            self.client = TradingClient(
                api_key=config['api']['alpaca']['api_key'],
                secret_key=config['api']['alpaca']['secret_key'],
                paper=True  # Automatically uses paper trading endpoint
            )
            self.logger.info("Alpaca paper trading client initialized successfully")
        except Exception as e:
    ...
    def get_account_info(self) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Account info retrieved - Cash: ${info['cash']:.2f}, Buying Power: ${info['buying_power']:.2f}")
            
            return info
            
        except Exception as e:
            self.logger.error(f"Failed to get account info: {e}")
            # Return safe defaults for paper trading
            return {
                'cash': 100000.0,  # Default paper trading balance
                'equity': 100000.0,
    ...
    def get_tradable_assets(self, symbol: str = None) -> List[Dict]:
        """Get list of tradable assets from Alpaca."""
        try:
            # Set up search parameters for US equities
            ...
            tradable_assets = [
                ...
                for asset in assets
                if asset.tradable  # Only include tradable assets
            ]
            
            # If symbol is provided, filter for that symbol
            if symbol:
                tradable_assets = [
                    asset for asset in tradable_assets 
                    if asset['symbol'] == symbol
                ]
                if tradable_assets:
                    self.logger.info(f"Asset {symbol} is tradable")
                else:
            ...
            self.logger.error(f"Failed to get tradable assets: {e}")
            return []

    def validate_symbol(self, symbol: str) -> bool:
        """Check if a symbol is valid and tradable."""
        assets = self.get_tradable_assets(symbol)
        return len(assets) > 0 and assets[0]['tradable']
